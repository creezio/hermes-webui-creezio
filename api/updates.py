...
def _creezio_update_check_uses_upstream() -> bool:
    """Creezio VPS: compare update banner against official upstream, not the fork."""
    flag = os.getenv('HERMES_WEBUI_DISABLE_SELF_UPDATE', '').strip().lower()
    return flag in {'1', 'true', 'yes', 'on'}


def _self_update_disabled_message() -> str | None:
    """Creezio installs disable in-UI self-update; merge via hermes-upstream-sync-creezio."""
    if _creezio_update_check_uses_upstream():
        return (
            'Mise à jour automatique désactivée sur cette install Creezio. '
            'Comparer upstream et fusionner via le skill hermes-upstream-sync-creezio.'
        )
    return None


def _update_fetch_remote(path: Path) -> str:
    """Return git remote used for update checks (origin fork vs upstream official)."""
    if _creezio_update_check_uses_upstream():
        url, ok = _run_git(['remote', 'get-url', 'upstream'], path)
        if ok and url.strip():
            return 'upstream'
    return 'origin'


def _official_upstream_compare_ref(path: Path, name: str) -> str:
    branch = 'main' if name == 'agent' else 'master'
...
def _check_repo_branch(path, name, *, fetch=True):
    ...
    remote = _update_fetch_remote(path)
    # Fetch latest from remote (network call, cached by TTL)
    if fetch:
        _, fetch_ok = _run_git(['fetch', remote, '--quiet', '--tags', '--force'], path, timeout=15)
        if not fetch_ok:
            return {'name': name, 'behind': 0, 'error': 'fetch failed'}

    if _creezio_update_check_uses_upstream() and remote == 'upstream':
        compare_ref = _official_upstream_compare_ref(path, name)
    else:
...
def _check_repo(path, name, channel=DEFAULT_UPDATE_CHANNEL):
    ...
    # See #2756.
    fetch_remote = _update_fetch_remote(path)
    fetch_out, fetch_ok = _run_git(['fetch', fetch_remote, '--tags', '--force'], path, timeout=15)
    if not fetch_ok:
...
def apply_force_update(target: str, channel=None) -> dict:
    ...
    """
    disabled = _self_update_disabled_message()
    if disabled:
        return {'ok': False, 'message': disabled, 'disabled': True}
    if channel is None:
        channel = _read_update_channel()
    channel = _normalize_channel(channel)
    blocker_snapshot = _restart_blocker_snapshot()
